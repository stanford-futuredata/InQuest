...
do
    echo "------ Running baselines + InQuest on dataset ${dataset} ------"
    echo "--- ${dataset} uniform ---"
    python simulator.py configs/final-uniform-configs/${dataset}-uniform-predicate-gt0-mean.json --trials-per-oracle-limit ${TRIALS_PER_ORACLE_LIMIT} --num-processes ${NUM_PROCESSES} --results-dir results-uniform-${dataset}
    echo "--- ${dataset} static ---"
    python simulator.py configs/final-static-configs/${dataset}-static-predicate-gt0-mean.json --trials-per-oracle-limit ${TRIALS_PER_ORACLE_LIMIT} --num-processes ${NUM_PROCESSES} --segments 4 --results-dir results-static-${dataset}
    echo "--- ${dataset} InQuest ---"
    python simulator.py configs/final-inquest-configs/${dataset}-inquest-predicate-gt0-mean.json --trials-per-oracle-limit ${TRIALS_PER_ORACLE_LIMIT} --num-processes ${NUM_PROCESSES} --segments 5 --alpha 0.8 --results-dir results-inquest-${dataset}
done
